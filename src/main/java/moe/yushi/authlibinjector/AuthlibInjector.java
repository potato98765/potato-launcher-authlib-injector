...
import moe.yushi.authlibinjector.transform.support.PotatoAuth121ClientTransformUnit;
import moe.yushi.authlibinjector.transform.support.PotatoAuth121Profiles;
...
public final class AuthlibInjector {
	...
	private static ClassTransformer createTransformer(APIMetadata config) {
		...
		transformer.units.add(new PotatoAuth121ClientTransformUnit());
		log(INFO, "PotatoAuth 1.21.x client transform enabled (profiles=" + PotatoAuth121Profiles.all().size() + ")");
